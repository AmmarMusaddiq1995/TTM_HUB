...
export function BookKeepingServicesFormMediumBusiness() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const emailSubject = encodeURIComponent("Book Keeping Services (Medium Business) - Service Request");
      const emailBody = encodeURIComponent(`
Dear Faaz Financial Group,

I am interested in your Book Keeping Services for Medium Business. Please find my details below:
      ...
      const { error } = await supabase.from("form_submissions").insert([
        {
          ...
          service_name: "Book Keeping Services (Medium Business)",
    ...
    <Card className="relative z-10 max-w-2xl mx-auto shadow-xl shadow-black">
      <CardHeader>
        <CardTitle className="text-lg font-bold text-center">
          Start Your Book Keeping Services (Medium Business)
        </CardTitle>
        <CardDescription className="text-center">
          Fill out the form below to begin your Book Keeping Services (Medium Business) process
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Button type="submit" className="w-full bg-[#2bb673] hover:bg-[#2bb673]/80 shadow-md shadow-black hover:scale-105 cursor-pointer px-8 py-4" disabled={loading}>
            {loading ? "Submitting..." : "Get Quotation For Book Keeping Services (Medium Business)"}
          </Button>
        </form>
      </CardContent>
    </Card>
