...
export function AdminOverview2() {
  ...
  const calculateCounts = (data) => {
    ...
    setCounts({ pending, inProgress, completed, totalRevenue, providerShare });
  };
 ...
 return () => {
  ...
  /* ---------------- Working Mark Status as Completed --------- */
  ...
  //       await axios.post("/api/mark-completed", { formId: id });
  ...
  const handleMarkStatusAsCompleted = async (id) => {
    try {
      // Block marking as completed if no admin file has been uploaded
      const hasUploadedFile = (() => {
        const fileField = selectedSubmission?.admin_uploaded_file;
        ...
        try {
          ...
          // If parsed but not array, treat truthy as present
          return Boolean(parsed);
        } catch {
          // Fallback for old single file URL format
          return typeof fileField === "string" && fileField.trim().length > 0;
        }
      })();
  
      if (!hasUploadedFile) {
        toast.error("Please upload at least one file before marking as completed.");
        return;
      }
      ...
      // Step 2: Trigger email API
      const res = await axios.post("/api/mark-completed", { formId: id });
  
      if (res.data.success) {
        toast.success("Status marked as completed and email sent!");
      } else {
        toast.error("Status updated, but email could not be sent. Please try again.");
      }
    } catch (err) {
      console.error(err);
          ...
          <table className="min-w-full text-sm text-left border-collapse">
            <thead className="bg-gray-100 text-gray-700 uppercase text-xs">
              <tr>
                ...
                <th className="px-4 py-3 border-b">Provider Share</th>
                <th className="px-4 py-3 border-b">Owner Share</th>
                <th className="px-4 py-3 border-b">Provider Paid</th>
                    ...
                    Object.entries(selectedSubmission.form_data).map(
                      ([key, value]) => (
                        <tr key={key} className="border-b last:border-0">
                          ...
                          <td className="py-2 px-3 text-gray-700 wrap-break-words w-2/3">
